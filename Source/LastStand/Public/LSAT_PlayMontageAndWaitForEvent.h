...
#include "LSAT_PlayMontageAndWaitForEvent.generated.h"
class ULSAbilitySystemComponent;

/** Delegate type used, EventTag and Payload may be empty if it came from the montage callbacks */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FLSPlayMontageAndWaitForEvent, FGameplayTag, EventTag, FGameplayEventData, EventData);

/**
* GOT IT FROM https://github.com/tranek/GASDocumentation#concepts-at-using
...
UCLASS()
class LASTSTAND_API ULSAT_PlayMontageAndWaitForEvent : public UAbilityTask
{
	GENERATED_BODY()
	
public:
	// Constructor and overrides
	ULSAT_PlayMontageAndWaitForEvent(const FObjectInitializer& ObjectInitializer);
	/**
...
*/
	...
	FLSPlayMontageAndWaitForEvent OnCompleted;

	/** The montage started blending out */
	UPROPERTY(BlueprintAssignable)
	FLSPlayMontageAndWaitForEvent OnBlendOut;

	/** The montage was interrupted */
	UPROPERTY(BlueprintAssignable)
	FLSPlayMontageAndWaitForEvent OnInterrupted;

	/** The ability task was explicitly cancelled by another ability */
	UPROPERTY(BlueprintAssignable)
	FLSPlayMontageAndWaitForEvent OnCancelled;

	/** One of the triggering gameplay events happened */
	UPROPERTY(BlueprintAssignable)
	FLSPlayMontageAndWaitForEvent EventReceived;
	...
	UFUNCTION(BlueprintCallable, Category = "Ability|Tasks", meta = (HidePin = "OwningAbility", DefaultToSelf = "OwningAbility", BlueprintInternalUseOnly = "TRUE"))
		static ULSAT_PlayMontageAndWaitForEvent* PlayMontageAndWaitForEvent(
